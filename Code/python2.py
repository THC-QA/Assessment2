...
def one(string1):
	return "".join([ch*3 for ch in string1])
...
def three(a):
	return sum([int(n) for n in [str(a)*n for n in range(1,5)]])
...
def four(string1, string2):
    return "".join([string1[n]+string2[n] for n in range(len(string1))])
...
def six(input):
	return True if input.lower()[-1] == "y" and input.lower()[-2] == "p" else False
...
def eight(string,  a):
	return string[:int(((len(string)-a)/2))] + string[int(-(((len(string)-a)/2))):] if len(string) != a else ""
...
def nine(string1, string3):
    list1 = [ch for ch in string1]
    list3 = [ch for ch in string3]
    for ch in string1:
        if ch in list3:
            list3.remove(ch)
    for ch in string3:
